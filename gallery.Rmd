...
```{r}
pigs_ts <- aus_livestock %>%
   filter(Animal=="Pigs",
          Month>=yearmonth('1990-01-01'),
          Month<=yearmonth('1995-12-01'),
          State=="Victoria")

### TODO: fix y axis with comma and add labels
pigs_ts %>%
  fpp3::gg_season(Count, labels="both", size=2, alpha=0.5) +
  base_plot_theme() +
...
prices %>%
  filter_index("1950" ~ .) %>%
  pivot_longer(cols =  c("eggs", "chicken", "copper", "nails", "oil", "wheat")) %>%
  gg_subseries(labels = "both")
```
...
state_stats %>% 
  ...
  ### geom_text(aes(label=abbr), size=3) +
  gghighlight(pop2010/1000000 > 10, label_key = abbr) 
```
